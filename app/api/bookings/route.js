...
export async function POST(req) {
  try {
    const { userId: sessionUserId, user } = await getSessionUser({
    ...
    const bookingUserId =
      hasRole(user, [ROLES.ADMIN]) && userId ? userId : user._id;
    ...
    try {
      ...
      console.log(
        "[Bookings] Service request notification created:",
        notification._id,
      );
    } catch (notifErr) {
      console.error(
        "[Bookings] Failed to create notification:",
        notifErr.message,
      );
    }
...
export async function GET(req) {
  try {
    ...
    if (isAdmin) {
      ...
      const ownProvider = await Provider.findOne({ clerkId: user.clerkId })
        .select("_id")
        .lean();
      if (!ownProvider?._id) {
        return NextResponse.json(
