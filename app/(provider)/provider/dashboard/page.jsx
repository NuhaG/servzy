    ...
    async function loadDashboard() {
      try {
        ...
        setStats({
          ...
          pending: bookingsData.filter((item) => item.status === "pending")
            .length,
        });
      } catch (err) {
      ...
      <div className="sv-shell space-y-5">
          ...
          <div className="flex items-center justify-between">
            <div className="flex-1">
              ...
              <p className="sv-subtitle mt-2">
                {provider
                  ? `${provider.businessName} | Status: ${provider.status}`
                  : "Loading provider..."}
              </p>
              {error ? (
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <div className="sv-card p-4">
            <p className="sv-subtitle">My Services</p>
            <p className="text-2xl font-bold">{stats.services}</p>
          </div>
          <div className="sv-card p-4">
            <p className="sv-subtitle">Bookings</p>
            <p className="text-2xl font-bold">{stats.bookings}</p>
          </div>
          <div className="sv-card p-4">
            <p className="sv-subtitle">Pending Requests</p>
